...
LOG_DIR=/home/logs
	...
	echo "----------------------------------------" >> $LOG_DIR/hdfsiox3-1.log
	echo " " >> $LOG_DIR/hdfsiox3-1.log
	echo Start in `date +%H:%M:%S--%d/%m/%Y` >> $LOG_DIR/hdfsiox3-1.log

	./hadoop jar ../share/hadoop/mapreduce/hadoop-mapreduce-client-jobclient-2.7.3-tests.jar TestDFSIO -write -nrFiles 48 -fileSize 512 2>&1>> $LOG_DIR/app_hdfsiox3-1.log 
	sleep 1
	echo End in `date +%H:%M:%S--%d/%m/%Y` >> $LOG_DIR/hdfsiox3-1.log
